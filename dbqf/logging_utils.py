...
class MLogger(object):
    def configure(self, file_name, logger_name, flush_secs=5, opt=None):
    ...
    def log_grads(self, grads, step):
        transformed_grads = self.tsne.fit_transform(grads.cpu())
        for i, grad in enumerate(transformed_grads):
            for j, dim in enumerate(grad):
                self.log_value('tsne/'+str(i)+'/'+str(j), dim, step)
