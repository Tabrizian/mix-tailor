...
def get_loaders(opt, global_rank=None):
    if opt.dataset == 'mnist':
        return get_mnist_loaders(opt, global_rank)
    elif opt.dataset == 'cifar10':
        return get_cifar10_loaders(opt, global_rank)
    elif opt.dataset == 'cifar100':
        return get_cifar100_loaders(opt, global_rank)
    elif opt.dataset == 'svhn':
        return get_svhn_loaders(opt, global_rank)
    elif opt.dataset.startswith('imagenet'):
        return get_imagenet_loaders(opt, global_rank)
    elif opt.dataset == 'logreg':
        return get_logreg_loaders(opt, global_rank)
    elif 'class' in opt.dataset:
        return get_logreg_loaders(opt, global_rank)


class HetrSampler(torch.utils.data.sampler.Sampler):

    def __init__(self, data_source, label):
        self.data_source = data_source
        self.label = label
        self.indexes = []
        self.total_classes = len(torch.unique(torch.tensor(data_source.ds.targets)))

        for i, (data, target, _) in enumerate(data_source):
            if target == (label) % self.total_classes:
...
def dataset_to_loaders(train_dataset, test_dataset, opt, global_rank=None):
    kwargs = {'num_workers': opt.workers,
              'pin_memory': True} if opt.cuda else {}
    idxdataset = IndexedDataset(train_dataset, opt, train=True)

    train_sampler = HetrSampler(idxdataset, global_rank) \
        if global_rank else None

    # train_sampler = DistributedSampler(idxdataset) \
    #     if not opt.heter and opt.distributed else train_sampler
...
def get_mnist_loaders(opt, global_rank=None):
    ...
    return dataset_to_loaders(train_dataset, test_dataset, opt, global_rank)
...
def get_cifar10_loaders(opt, global_rank=None):
...
def get_svhn_loaders(opt, global_rank=None):
...
def get_imagenet_loaders(opt, global_rank=None):
    # Data loading code
